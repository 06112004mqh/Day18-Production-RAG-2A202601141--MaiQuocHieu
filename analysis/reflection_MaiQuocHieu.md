...
| **Cosine / Cross-Encoder Reranking** | M3 | `CrossEncoderReranker.rerank()` | Rerank danh sách candidate top-20 từ Hybrid Search xuống Top 3. Sử dụng OpenAI vector cosine scoring giúp loại bỏ noise và nâng điểm Context Precision lên 90-91%. |
| **RAGAS 4 Metrics Evaluation** | M4 | `evaluate_ragas()` | Đánh giá tự động pipeline theo 4 tiêu chí chuẩn: Faithfulness (0.7183), Answer Relevancy (0.6402), Context Precision (0.9000), Context Recall (0.7167). |
